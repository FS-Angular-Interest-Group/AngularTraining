...
function selector(stream) {
	return ident(stream) || atKeyword(stream) || className(stream) || id(stream)
		|| pseudo(stream) || eatAttribuite(stream) || combinator(stream);
}

function value(stream) {
	return url(stream) || string(stream) || number(stream) || hash(stream)
		|| keyword(stream) || important(stream) || operator$1(stream);
}
...
function createProperty(stream, tokens, terminator) {
	...
	let separator, value$$1, ix = 0;
	const name = tokens[ix++];

	if (ix < tokens.length) {
		value$$1 = tokens[ix++];
		value$$1.type = 'value';
		value$$1.end = last(tokens).end;
	}

	if (name && value$$1) {
		separator = new Token(stream, 'separator', name.end, value$$1.start);
	}
...
class Property extends Node {
	constructor(stream, name, value$$1, separator, terminator) {
		super('property');
		this.stream = stream;
		this.nameToken = name;
...
function parseStylesheet(source) {
	...
	while (!stream.eof()) {
		...
		if (stream.eatWhile(PROP_DELIMITER)) {
			...
			tokens.length = 0;
		} else if (eatUrl(stream) || eatBraces(stream, root) || eatString$1(stream) || stream.next()) {
